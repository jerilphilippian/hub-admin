...
<div class = "px-2 pt-20 pb-4 ml-12 duration-500 ease-in-out transform content md:px-5 ">
    @include('layouts.group-management-menu')
...
<ul class="inline-flex items-center gap-6 px-5 py-3 my-6 mb-2 text-xs font-semibold tracking-wider text-black bg-white shadow rounded-3xl">
    <a href="{{route('group-management')}}" class="relative nav active transition duration-300 ease-in-out hover:-translate-y-1 {{ request()->routeIs('events.index') ? 'text-red-500 ' : '' }}">
        <li class="font-bold capitalize">Group List</li>
        <div class="nav-underline"></div>
    </a>
    <a href="{{route('postlist.postlist')}}" class="relative nav active transition duration-300 ease-in-out hover:-translate-y-1 {{ request()->routeIs('events.index') ? 'text-red-500 ' : '' }}">
        <li class="font-bold capitalize">Post List</li>
        <div class="nav-underline"></div>
    </a>
</ul>
